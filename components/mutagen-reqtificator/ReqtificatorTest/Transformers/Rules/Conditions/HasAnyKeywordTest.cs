...
{
    public class HasAnyKeywordTest
    {
        ...
        public void Should_match_records_that_have_all_keywords()
        {
            ...
            var condition = new HasAnyKeyword<IArmorGetter>(ImmutableHashSet<IFormLinkGetter<IKeywordGetter>>.Empty
                .Add(Keyword1).Add(Keyword2));
            condition.CheckRecord(armor).Should().BeTrue();
        }

        [Fact]
        public void Should_match_records_that_have_only_a_subset_of_the_required_keywords()
        {
            var armor = new Armor(FormKey.Factory("123456:Requiem.esp"), SkyrimRelease.SkyrimSE)
            {
