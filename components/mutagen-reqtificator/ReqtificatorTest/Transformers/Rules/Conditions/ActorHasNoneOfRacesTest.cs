...
{
    public class ActorHasNoneOfRacesTest
    {
        ...
        [Fact]
        public void Should_match_an_actor_with_a_single_inheritance_and_none_of_the_disallowed_races()
        {
            ...
            var condition = new ActorHasNoneOfRaces(new HashSet<IFormLinkGetter<IRaceGetter>>
                {Race1, Race2}.ToImmutableHashSet(), graph.Object);
            condition.CheckRecord(actor).Should().BeTrue();
        }

        [Fact]
        public void Should_match_an_actor_with_multiple_inheritances_each_having_none_of_the_disallowed_races()
        {
            var graph = new Mock<IActorInheritanceGraphParser>(MockBehavior.Strict);
            var actor = new Npc(FormKey.Factory("123456:Requiem.esp"), SkyrimRelease.SkyrimSE)
            {
        ...
        [Fact]
        public void Should_not_match_an_actor_with_a_single_inheritance_and_one_of_the_disallowed_races()
        {
            var graph = new Mock<IActorInheritanceGraphParser>(MockBehavior.Strict);
            var actor = new Npc(FormKey.Factory("123456:Requiem.esp"), SkyrimRelease.SkyrimSE)
            {
